        ...
        return new Promise((resolve, reject) => {
            const getPassword = (connection, user_id) => {
                return new Promise((resolve, reject) => {
                    ...
                    connection.query("SELECT password, HEX(salt) AS salt, HEX(salt_iv) AS salt_iv FROM passwd WHERE user_id = UNHEX(" + connection.escape(user_id) + ")",
                    (error, results, fields) => {
                        // Close the connection
                        connection.end();
            ...
            connection.query("SELECT HEX(user_id) AS user_id FROM users WHERE email_address = " + connection.escape(email),
            (error, results, fields) => {
                if (error){
                    // Close the connection
                    connection.end();
...
module.exports.User = class User {
    ...
    loadInfo(){
        return new Promise((resolve, reject) => {
            ...
            connection.query("SELECT * FROM users WHERE user_id = UNHEX(" + connection.escape(this.user_id) + ")",
            (error, results, fields) => {
                // Close the connection
                connection.end();
