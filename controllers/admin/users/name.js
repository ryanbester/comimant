...
exports.changeName = (req, res) => {
    ...
    Nonce.verifyNonce('admin-users-user-name-form', nonce, Util.getFullPath(req.originalUrl)).then(_ => {
        ...
        if (invalidFields.length > 0) {
            renderPage(req, res, invalidFields.length + ' fields are invalid', invalidFields, firstName,
                lastName);
            return;
        }
        ...
        targetUser.saveUser().then(_ => {
            renderPage(req, res, undefined, undefined, firstName, lastName,
                'Successfully saved the user\'s name');
        }, _ => {
            Logger.debug(
                Util.getClientIP(
                    req) + ' tried to access page admin.users.change_name but failed to save to database.');
            renderPage(req, res, 'Error saving the user\'s name. Please try again.');
        });
    }, _ => {
        Logger.debug(
